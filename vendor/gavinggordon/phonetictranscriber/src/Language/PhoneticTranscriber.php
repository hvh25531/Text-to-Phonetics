...
class PhoneticTranscriber
{
	...
	public function transcribe( $text = NULL )
	{
		...
		if( isset( $this->data['text_to_transcribe'] ) && is_string( $this->data['text_to_transcribe'] ) && strlen( $this->data['text_to_transcribe'] ) >= 1 )
		{
			...
			// Custom
			preg_match_all( '/(<span class="transcription_missing">(.+)<\/span>)?(.+)?<span\s+class="transcribed[\_]word">(<a.+?\>)?(.+)(<\/a>)?<\/span>/i', $transcription, $matches );

			$result = preg_replace('/<a.+?HideContent\(\)">/i', '', $matches[0]);
			...
			if( $result )
			{
				...
				switch( $this->data['output_style'] )
				{
					case 'text':
						...
						break;
					case 'columns':
						$resultText = $this->data['text_to_transcribe'] . '   ' . $result;
						break;
					case 'inline':
